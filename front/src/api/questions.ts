...
export const fetchTodayQuestion = async (matchId: number): Promise<TodayQuestion> => {
  const res = await instance.get(`/matches/${matchId}/questions/today`);
  return res.data;
};

// 답변 하기
export const answerQuestion = async (questionInstanceId: number, content: string) => {
  const res = await instance.post(`/question-instances/${questionInstanceId}/answers`, {
    content,
  });
  return res.data;
};
...
export const updateAnswer = async (answerId: number, content: string) => {
  const res = await instance.patch(`/answers/${answerId}`, { content });
  return res.data;
};

//질문 평가
export const ratingQuestion = async (questionId: number, isLike: boolean) => {
  const res = await instance.post(`/questions/${questionId}/ratings`, { isLike });
  return res.data;
};
