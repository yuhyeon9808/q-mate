...
export const fetchCustomQuestions = async (
  ...
  const res = await instance.get(`/matches/${matchId}/custom-questions`, {
    params: {
      page,
      size,
      status: 'EDITABLE',
...
export const createCustomQuestion = async ({
  ...
  const res = await instance.post(`/matches/${matchId}/custom-questions`, { text });
  return res.data;
};

// 커스텀 질문 수정
export const updateCustomQuestion = async ({ text, id }: { text: string; id: number }) => {
  const res = await instance.patch(`/custom-questions/${id}`, { text });
  return res.data;
};

// 커스텀 질문 삭제
export const deleteCustomQuestion = async (id: number) => {
  const res = await instance.delete(`/custom-questions/${id}`);
  return res.data;
};
